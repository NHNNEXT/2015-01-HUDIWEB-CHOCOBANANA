...
public class TodoDao {
	...
	public ArrayList<TodoEntity> getPersonalTodos(Long uid) {
		...
		List<Map<String, Object>> rows = jdbcTemplate.queryForList(QrysT.RETRIEVE_TODO_FOR_PERSONAL_VIEW, uid);
	...
	public int insertTodo(TodoEntity todo) {
		...
		return jdbcTemplate.update(QrysT.INSERT_TODO, todo.getPid(), todo.getTitle(), todo.getContents(), todo.getDueDate(), todo.getEditerId());
	}
	...
	public TodoEntity getLastTodo (Long tid) {
		...
		RowMapper<TodoEntity> rowMapper = new RowMapper<TodoEntity>() {
			public TodoEntity mapRow(ResultSet resultSet, int rowNum) {
				try {
					return new TodoEntity(resultSet.getLong("tid"), resultSet.getLong("pid"), resultSet.getString("title"), resultSet.getString("contents"),
	...
	public int insertHistory (Long tid, TodoEntity result) {
		...
		return jdbcTemplate.update(QrysT.INSERT_HISTORY, tid,  result.getPid(), result.getTitle(),  result.getContents(), result.getDueDate(), result.getStatus(), result.getEditerId());
	}
