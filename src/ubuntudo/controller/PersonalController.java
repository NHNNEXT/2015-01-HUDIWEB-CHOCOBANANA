...
public class PersonalController {
	...
	public @ResponseBody TodoEntity addPersonalTodo(HttpSession session,
			HttpServletRequest req) throws ServletRequestBindingException {
		logger.debug("/personal/todo POST요청에 대해 응답");
		if (session.getAttribute("user") == null) {
			logger.debug("/personal 요청에 대해 응답 - 세션이 정상적이지 않을때");
			// return "redirect:/start"; string으로 보내야하는데... 어쩐다... -> 나중에 주기적으로
			// ajax요청 보낼 때 체크해서 redirect해주면 될 듯?!
		}
		UserEntity user = (UserEntity) session.getAttribute("user");
		Long uid = user.getUid();
		Long pid = ServletRequestUtils.getRequiredLongParameter(req, "pid");
		String title = ServletRequestUtils.getRequiredStringParameter(req,
				"title");
		String contents = ServletRequestUtils.getRequiredStringParameter(req,
				"contents");
		String duedate = ServletRequestUtils.getRequiredStringParameter(req,
				"dueDate");
