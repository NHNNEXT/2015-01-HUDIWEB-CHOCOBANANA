...
public class QrysG extends Qrys {
	...
	public static final String RETRIEVE_GUILD_DETAIL = "select g.gid, g.g_name, g.leader_id, u.name, (select count(*) from user_guild_relation where gid = ?) members, (select count(*) from party where gid = ?) parties from guild g join user u on g.leader_id = u.uid where g.gid = ?";
}